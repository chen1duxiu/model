...
class scm_layer(nn.Layer):
    def __init__(self, in_channels, key_channels):
        ...
        # or use --self.f_pixel = layers.ConvBNReLU(in_channels, key_channels, 1) or use --self.f_pixel =
        # nn.Sequential(layers.ConvBNReLU(in_channels, key_channels, 1),layers.ConvBNReLU(key_channels, key_channels,
        # 1))
        self.f_query = layers.ConvBNReLU(in_channels, key_channels, 3, 1)

        # or use --self.f_object = nn.Sequential(layers.ConvBNReLU(in_channels, key_channels, 1),layers.ConvBNReLU(
        # key_channels, key_channels, 1))
