...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    # 最后一个卷积的输出通道数
    inplanes = 2048
    # 将骨干网络中的低层特征与 ASPP 模块中的高层特征进行融合时，使用的卷积核数量为 256
    low_level_planes = 256

    if name=='deeplabv3plus':
        return_layers = {'layer4': 'out', 'layer1': 'low_level'}
        # 'layer4' 表示骨干网络中的最后一个卷积层，'out' 表示该卷积层的输出特征图。
        # 'layer1' 表示骨干网络中的第一个卷积层，'low_level' 表示该卷积层的输出特征图
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name=='deeplabv3plus2':
        return_layers = {'layer4': 'out', 'layer1': 'low_level'}
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name=='deeplabv3':
...
def deeplabv3plus2_resnet50(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3 model with a ResNet-50 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus2', 'resnet50', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)
